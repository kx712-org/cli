...
import { selectMenu, promptText, confirm, pause, clearScreen } from "./tui.js";
...
async function pickOne(list, title) {
    if (list.length === 0) {
        console.log("(aucun élément)");
        await pause();
        return null;
    }
    const items = list.map((x) => ({ label: x.name, value: x.id, hint: x.id }));
...
async function authMenu() {
    for (;;) {
        ...
        if (!choice || choice === "back")
            return;
        if (choice === "status")
            await run(async () => whoami());
        else if (choice === "login")
            await run(() => login([]));
        else if (choice === "logout")
            await run(async () => logout());
    }
}
async function projectsMenu() {
    ...
    for (;;) {
        ...
        if (choice === "list") {
            ...
            if (!(await confirm(`Supprimer « ${target?.name ?? id} » ? Action irréversible.`)))
                continue;
...
async function serviceActionsMenu(id, name) {
    for (;;) {
        ...
        if (choice === "info") {
            ...
            if (await confirm("Lancer un redéploiement ?"))
                await run(() => services(["redeploy", id]));
        }
        else if (choice === "restart") {
            if (await confirm("Redémarrer ce service ?"))
                await run(() => services(["restart", id]));
        }
        else if (choice === "stop") {
            if (await confirm("Arrêter ce service ?"))
                await run(() => services(["stop", id]));
        }
            ...
            if (await confirm("Revenir à un déploiement antérieur (rollback) ?")) {
                const deployments = await apiFetch(`/api/services/${id}/deployments`);
