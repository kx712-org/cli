...
export async function selectMenu(title: string, items: MenuItem[]): Promise<string | null> {
...
export async function promptText(question: string, opts?: { default?: string }): Promise<string> {
...
export async function pause(message = "Entrée pour continuer…"): Promise<void> {
  const iface = rl.createInterface({ input: process.stdin, output: process.stdout });
