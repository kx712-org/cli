...
export async function services(argv: string[]): Promise<void> {
  const [sub, ...rest] = argv;
  ...
  switch (sub) {
    ...
    case undefined: {
      ...
      const data = await apiFetch<Service[]>(path);
      if (isJsonMode(argv)) return printJson(data);
      table(
        data.map((s) => ({ id: s.id, name: s.name, type: s.type ?? "—", status: s.status ?? "—" })),
        [
          { key: "id", label: "ID" },
          { key: "name", label: "NOM" },
          { key: "type", label: "TYPE" },
    ...
    case "info": {
      if (!id) fail("Usage : webgune services info <id>");
      const data = await apiFetch(`/api/services/${id}`);
      return printJson(data);
    }
    case "logs": {
      if (!id) fail("Usage : webgune services logs <id> [--tail <n>]");
      const tail = getFlag(argv, "--tail") ?? "200";
      const data = await apiFetch<{ logs?: string } | string[]>(`/api/services/${id}/logs?tail=${tail}`);
      if (Array.isArray(data)) data.forEach((l) => console.log(l));
      else if (typeof data === "object" && data.logs) console.log(data.logs);
      else printJson(data);
      return;
    }
    case "redeploy": {
      if (!id) fail("Usage : webgune services redeploy <id>");
      await apiFetch(`/api/services/${id}/redeploy`, { method: "POST" });
      console.log("Redéploiement lancé.");
      return;
    }
    case "restart": {
      if (!id) fail("Usage : webgune services restart <id>");
      await apiFetch(`/api/services/${id}/restart`, { method: "POST" });
      console.log("Redémarrage lancé.");
      return;
    }
    case "stop": {
      if (!id) fail("Usage : webgune services stop <id>");
      await apiFetch(`/api/services/${id}/stop`, { method: "POST" });
      console.log("Service arrêté.");
      return;
    }
    case "start": {
      if (!id) fail("Usage : webgune services start <id>");
      await apiFetch(`/api/services/${id}/start`, { method: "POST" });
      console.log("Service démarré.");
      return;
    }
    case "deployments": {
      if (!id) fail("Usage : webgune services deployments <id>");
      const data = await apiFetch<Deployment[]>(`/api/services/${id}/deployments`);
      if (isJsonMode(argv)) return printJson(data);
      table(
        data.map((d) => ({
          id: d.id,
    ...
    case "rollback": {
      ...
      if (!id || !deploymentId) fail("Usage : webgune services rollback <serviceId> <deploymentId>");
    ...
    default:
      fail(
        `Sous-commande inconnue : services ${sub}\n` +
